...
void C4ConfigNetwork::CompileFunc(StdCompiler *compiler)
{
	...
	compiler->Value(mkNamingAdapt(s(AlternateServerAddress),"AlternateServerAddress", "https://league.planova.org/league.php"));
	compiler->Value(mkNamingAdapt(UseAlternateModDatabaseServer, "UseAlternateModDatabaseServer", 0));
	compiler->Value(mkNamingAdapt(s(AlternateModDatabaseServerAddress), "AlternateModDatabaseServerAddress", "https://mods.planova.org/api/"));
	compiler->Value(mkNamingAdapt(s(LastPassword),         "LastPassword",         "Wipf"        ));
#ifdef WITH_AUTOMATIC_UPDATE
	compiler->Value(mkNamingAdapt(s(UpdateServerAddress),  "UpdateServerAddress",     "https://www.planova.org/update/"));
	...
	compiler->Value(mkNamingAdapt(s(PuncherAddress),       "PuncherAddress",       "netpuncher.planova.org:11115"));
...
void C4ConfigIRC::CompileFunc(StdCompiler *compiler)
{
	...
	compiler->Value(mkNamingAdapt(s(Channel),              "Channel",              "#planova"    , false, true));
...
const char* C4ConfigNetwork::GetLeagueServerAddress()
{
	...
	{
		return "https://league.planova.org/league.php";
	}
}
...
const char* C4ConfigNetwork::GetModDatabaseServerAddress()
{
	...
	// Standard (hardcoded) official mod database server
		...
		return "https://mods.planova.org/api/";
}
