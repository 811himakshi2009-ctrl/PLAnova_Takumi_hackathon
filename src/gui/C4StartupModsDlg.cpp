 ...
 *
 * Copyright (c) 2009-2017, The PLAnova Team and contributors
 *
 *
 *
 */
...
void C4StartupModsListEntry::FromXML(const TiXmlElement *xml, ModXMLData::Source source, std::string fallbackID, std::string fallbackName)
{
	...
	static std::string planovaVersionStringTag;
	if (planovaVersionStringTag.empty())
		planovaVersionStringTag = C4StartupModsDlg::GetPLAnovaVersionStringTag();
	...
	for (auto &tag : modXMLData->tags)
	{
		...
		else if (tag == planovaVersionStringTag)
...
std::string C4StartupModsDlg::GetPLAnovaVersionStringTag()
{
	return "planova-" + std::to_string(C4XVER1);
}
...
void C4StartupModsDlg::QueryModList(bool loadNextPage)
{
	...
	{
		static const std::string versionTag = GetPLAnovaVersionStringTag();
