...
if ($env:DEPLOYMENT_URL -and $env:DEPLOYMENT_SECRET) {
    ...
    $planova = $projects.LoadedProjects | ?{ $_.GetPropertyValue('ProjectName') -eq 'planova' }
    ...
    if ($planova -and $c4group -and $groups) {
    ...
    # Copy executables, Qt
    C:\Qt\5.11\msvc2017_64\bin\windeployqt.exe --no-translations --no-compiler-runtime --dir $deploy_dir $planova.GetPropertyValue('TargetPath')
    Copy-Item $planova.GetPropertyValue('TargetPath') "${deploy_dir}\"
    Copy-Item $c4group.GetPropertyValue('TargetPath') "${deploy_dir}\"

    # Copy other DLLs planova depends on
    function Get-Imports {
        param([string]$file)
        (dumpbin /imports $file | ?{ $_ -match ' {4}[\w-]+\.dll'}).Trim()
    }
    $unresolved = [System.Collections.Queue]::new()
    Get-Imports $planova.GetPropertyValue('TargetPath') | %{ $unresolved.Enqueue($_) }
    ...
    # Create archive
    $archive_name = "PLAnova-win-${env:PLATFORM}.zip"
